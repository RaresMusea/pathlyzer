...
export const ExaminationComponent = () => {
    ...
    useEffect(() => {
        if (!pathname.includes("/quiz")) return;
    ...
    }, [pathname]); // IMPORTANT: refacem listeners când se schimbă pathname
    ...
    return (
        <div>
            <AnimatePresence>
                ...
                {modals.complete && (
                    <ExaminationCompleteModal
                        examinationType={examinationType}
                        currentXp={xp}
                        isLastLesson={isLastFromUnit}
                        currentLevel={level}
                    ...
                    >
                        ...
                        {currentQuestion?.type === QuestionType.CODE_FILL && (
                            <CodeFillQuestion
                                ...
                                codeFillEvaluations={codeFillEvaluations}
                                handleCodeFillAnswer={handleCodeFillAnswer}
                            />
                        )}
                    </motion.div>
                </main>
                ...
                <footer
                    key="examinationFooter"
                    className={cn(
                        "border-t py-4 px-6 shadow-md w-full absolute bottom-0 font-nunito bg-primary-foreground"
                    )}
                >
                                    ...
                                    {answerStatus.isChecked ? (
                                        answerStatus.wasCorrect ? (
                                            <span className="flex items-center gap-2">
                                                <CircleCheck className="w-4 h-4 mr-2" /> Corect! Well
                                                done!
                                            </span>
                                        ) : (
                                            <span className="flex items-center gap-2">
                                                <CircleX className="w-4 h-4 mr-2" /> Wrong! Please try
                                                again
                                            </span>
                                        )
                                    ) : (
                                        <span className="flex items-center gap-2">
                                            <ListCheck className="w-4 h-4 mr-2" /> Check answer
                                        </span>
                                    )}
                                </Button>
                            )}
